...
def tree_insert( tree, item): #function to insert each item into the binary seach tree
    ...
    if tree==None: #if tree is empty...
        tree=BinTreeNode(item) #...tree is equal to the item input
    else:
...
def in_order(tree): #takes the BST as input and traverses it to return the values in order
    ...
    while node != None or len(stack) >0:
        if node != None: #if node is not None, append it to the stack
            stack.append(node)
            node=node.left #node is set to node.left
        else:
            node = stack.pop() #if no node to the left then node is last item in stack
            solution.append(node.value) #node added to solution
            node=node.right
